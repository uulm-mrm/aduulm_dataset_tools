...
###################################################################################################################################
### This script creates the ground-truth images for drivable area detection from a given json-file of the corresponding image.  ###
...
###     - argument 2: subtask ('full' or 'simple')                                                                              ###
###     - argument 3: training_set ('train','val','test', 'all')                                                                ###
###                                                                                                                             ###
###                                                                                                                             ###
### usage: python prepare_TrainingData_DrivableArea.py <path2dataset> <subtask> <training_set>                                  ###
### e.g. python prepare_TrainingData_DrivableArea.py /home/user/ADUUlmataset full train                                         ###
###                                                                                                                             ###
### Version 1.0                                                                                                                 ###
...
if (len(sys.argv) != 4):
    ...
    print ("[ERROR] argument 1: <path2dataset> ")
    print ("[ERROR] argument 2: training_set ('train','val','test', 'all')")
    print ("[ERROR] python prepare_TrainingData_DrivableArea.py <path2dataset> <training_set> ")
    sys.exit()

# path to dataset
DATADIR_ADUULM_DATASET=sys.argv[1]

# subtask (choose between "full" (contains all 8 classes) and "simple" (contains the reduced class set of 4 classes))
#SUBTASK = "full"
SUBTASK=sys.argv[2]
...
path2list = os.path.join(DATADIR_ADUULM_DATASET, "image_splits/drivable_area_detection/", TRAINING_SET+".txt")
        ...
        if s[0] == '\"label\":': 
            ...
            if len(object_polygon) > 0:
                if name == "EgoLane":
                    ...
                    img_id = 1
                elif name == "ParallelLane":
                    color = np.array([255, 255, 0])
                    img_id = 2
                elif name == "OppositeLane":
                    color = np.array([255, 0, 0])
                    img_id = 3
                elif name == "CrossingArea":
                    color = np.array([102, 255, 255]) if SUBTASK == "full" else np.array([0, 255, 0])
                    img_id = 4 if SUBTASK == "full" else 1
                elif name == "ParallelTurnLane":
                    color = np.array([204, 204, 0]) if SUBTASK == "full" else np.array([255, 255, 0])
                    img_id = 5 if SUBTASK == "full" else 2
                elif name == "OppositeTurnLane":
                    color = np.array([102, 0, 102]) if SUBTASK == "full" else np.array([255, 0, 0])
                    img_id = 6 if SUBTASK == "full" else 3
                elif name == "ParkingLane":
                    color = np.array([0, 0, 255]) if SUBTASK == "full" else np.array([0,0,0])
                    img_id = 7 if SUBTASK == "full" else 0
                else: 
